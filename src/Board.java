...
public class Board extends JPanel {
    ...
    private void patinGeneralPath(int x, int y, Graphics2D gr2d, String data) {
    ...
    private void paintOver(AVLNode<Integer> current, int x, int y, boolean isLeft) {
        ...
        if (current == avlTree.getRoot()) {
            currentX = x;
            currentY = y;
            patinGeneralPath(x, y, g2, String.valueOf(avlTree.getRoot().data));
        } else if (isLeft) {
            currentX = x - 150+count;
            currentY = y + 60+count;
            patinGeneralPath(currentX, currentY, g2, String.valueOf(current.data));
